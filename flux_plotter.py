...
#arcsec/pix conversion
bit_a1689_scale = .140843
bit_cosmos_scale = .140732


# initializing values and methods
data = np.loadtxt('a1689/catalogs/band1_x_hscg_catalog.csv', delimiter = ',', skiprows=1)
psf = np.loadtxt("a1689/flux_plots/psf_cut.txt")
...
for obj in data:
    if (obj[12]>flux_med and bit_a1689_scale*obj[24]<.4): #greater than flux mean was once here
        pairs.append([bit_a1689_scale*obj[24],obj[12]])
pairs = np.array(pairs)

popt,pcov = curve_fit(psf_form, psf[:,0],psf[:,1], maxfev=100000)

pop, pcov = curve_fit(psf_func, pairs[:,0], pairs[:,1], p0 = [1, 1, 1], bounds = ([0,.0001, .001], [100, 100, 100]), maxfev=100000)

mod, mvar = curve_fit(model, pairs[:,0], pairs[:,1], p0 = [1, 5, 1, 1,0], bounds = ([0,.0001, .001, -100,-10], [100, 100, 100, 100,100]), maxfev=1000000)

'''
# plots of data with cuts drawn
plt.scatter(bit_cosmos_scale*data[:,24], data[:,12], color = 'blue', label = 'Non-PSF-fitting data')
plt.scatter(pairs[:,0], pairs[:,1], color = 'black', label = 'PSF-fitting data')
...
plt.scatter(bit_cosmos_scale*data[:,24], psf_func(bit_cosmos_scale*data[:,24], pop[0], pop[1], pop[2]), color = 'red', label = "Bill PSF fit cut")
...
bit_back = []
errs = []
manual_min_bit = 0.005 #0.005 
manual_max_bit = .07 #.07 for trying to get same regime for cosmos and a1689
manual_max_hsc = 15000 #for cosmos 1,2
...
outlier_list = np.loadtxt('a1689/flux_plots/outlier_files/c1g_outliers.csv', delimiter=',')
...
print('Survival rate = ' + str(bit_flux.size/total))
...
count = 0
for entry in ratios:
    if entry < med + thresh and entry > med - thresh:
        count = count + 1
print(count/ratios.size)



# region generator for outliers
