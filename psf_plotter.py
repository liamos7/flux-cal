...
# median of pixel values on far disk around point
...
# sum of pixel values in 12-pixel radius circle around point
...
def get_f(ra, dec):
    ...
    d_0 = data[int(y), int(x)] - get_background(ra, dec)  # value at obj barycenter, minus background
...
for obj in hsc_cat:
    ...
    if in_box and hsc_pix_to_arcsec*obj[11]>.22 and hsc_pix_to_arcsec*obj[11]<.24: #sure that stars lie here?
...
for star in hsc_star_cal:
    ...
    cut = .1
    if data[int(y),int(x)] > cut: #brighter than .1
